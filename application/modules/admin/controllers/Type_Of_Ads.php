...
class Type_Of_Ads extends MX_Controller {
    ...
    function request() {
        ...
        }else if ($this->uri->rsegment(3) == "upload_pdf") {
            self::request_upload_pdf();
        } 
    }
    ...
    public function multiple_files(){ 
        $client_name = urldecode($this->uri->rsegment(3));
        ...
        $start_billing = $this->uri->rsegment(9);
        $end_billing = $this->uri->rsegment(10);
        $get_location_name = Locations_Model::getFields($location_id);
        $get_type_of_ads_name = Type_Of_Ads_Model::getFields($type_of_ads_id);
        $location_name_fields = urldecode($get_location_name->location_name);
        ...
        $year = date("Y");
            ...
            for ($i = 0; $i < $count_uploaded_files; $i++) {
                ...
                $insert_data['client_contract_id'] = $get_client_contract_id_returned_data;
                        ...
                        $insert_data['start_billing'] = $start_billing;
                        $insert_data['end_billing'] = $end_billing;
                        $insert_data['returned'] = 0;
    ...
    public function request_upload_pdf(){ 
        $soa = $this->uri->rsegment(4);
        $start_billig = $this->uri->rsegment(5);
        $end_billing = $this->uri->rsegment(6);
        $contract_number = $this->uri->rsegment(7);
        $contract_client_id = $this->uri->rsegment(8);
        
        $year = date("Y");
            $config['upload_path'] = "./Uploads/pdf";
            $config['allowed_types'] = 'gif|jpg|png|jpeg|bmp|mp3|mp4|pdf';
            $config['max_filename'] = '255';
            $config['encrypt_name'] = FALSE;
            $config['max_size'] = '25000000';
            $F = array();
            ...
            for ($i = 0; $i < $count_uploaded_files; $i++) {
                ...
                $insert_data['client_contract_code'] = $contract_number;
                        $insert_data['monthly_start_billing'] = $start_billig;
                        $insert_data['monthly_end_billing'] = $end_billing;
                        $insert_data['soa'] = $soa;
                        $insert_data['client_contract_id'] = $contract_client_id;
                        $insert_data['pdf_file_name'] = $file_name;
                        $insert_data['enabled'] = 1;
                Saved_Photo_Model::insert_pdf($insert_data);
                $this->load->library('upload', $config);
                if (!$this->upload->do_upload('file')) {
                    $result['errors'][] = $this->upload->display_errors();
                } else {
