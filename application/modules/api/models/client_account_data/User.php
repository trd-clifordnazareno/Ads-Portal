...
class Client_Account_Data_Model extends MY_Model{
	...
	function getFields($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('cad.contract_client_id'=>$id));
		$query = $this->db->get();
	...
	function getcontractnumber($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('cad.contract_number'=>$id));
		$query = $this->db->get();
	...
	{
		$this->db->from("client_contract cad");
	}
	...
	{
		$this->db->where(array('cad.enabled' => 1));
	}
	...
	function insert_table($data){
		$this->db->insert("client_contract",$data);
	...
	function update_table($data,$table_col,$key){
		...
		$this->db->update("client_contract",$data);
        return $key;
	}
