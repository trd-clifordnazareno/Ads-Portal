...
class Contract_And_Locations_Model extends MY_Model{
	...
	function getFields($id){
		...
		self::_join();
		self::_fix_arg();
		parent::where(array('cal.Location_id'=>$id));
		$query = $this->db->get();
	...
	function check_client_contract_id_and_location_id($client_contract_id, $location_id){
		self::_select();	
		self::_from();
		self::_join();
		self::_fix_arg();
		parent::where(array('cal.Location_id'=>$client_contract_id, 'cal.client_contract_id'=>$location_id));
		$query = $this->db->get();
	...
	{
		$this->db->from("contarct_and_locations cal");
	}
	...
	{
		$this->db->where(array('cal.enabled' => 1));
	}
	...
	function insert_table($data){
		$this->db->insert("contarct_and_locations",$data);
	...
	*/
	function getFields_arr($where,$select,$return='',$return_value=''){
        ...
        public function delete_client_contract_locations($data, $client_contract_id, $location_id, $client_contract_id_key, $location_id_key){
            $this->db->where($client_contract_id,$client_contract_id_key);
            $this->db->where($location_id,$location_id_key);
		$this->db->update("contarct_and_locations",$data);
                $deleted_success = 1;
